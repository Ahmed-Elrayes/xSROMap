/**
 * xSROMap - Silkroad Online Map Library
 * Built on Leaflet.js
 * @author JellyBitz
 * @version 1.4.0
 */

import L from 'leaflet';

// Virtual marker performance enhancement
L.Marker.addInitHook(function () {
  if (this.options.virtual) {
    this.on('add', function () {
      this._updateIconVisibility = function () {
        if (this._map == null) return;
        const isVisible = this._map.getBounds().contains(this.getLatLng());
        const wasVisible = this._wasVisible;
        const icon = this._icon;
        let iconParent = this._iconParent;
        const shadow = this._shadow;
        let shadowParent = this._shadowParent;

        if (!iconParent) {
          iconParent = this._iconParent = icon.parentNode;
        }
        if (shadow && !shadowParent) {
          shadowParent = this._shadowParent = shadow.parentNode;
        }

        if (isVisible !== wasVisible) {
          if (isVisible) {
            iconParent.appendChild(icon);
            if (shadow) {
              shadowParent.appendChild(shadow);
            }
          } else {
            iconParent.removeChild(icon);
            if (shadow) {
              shadowParent.removeChild(shadow);
            }
          }
          this._wasVisible = isVisible;
        }
      };
      this._map.on('resize moveend zoomend', this._updateIconVisibility, this);
      this._updateIconVisibility();
    }, this);
  }
});

/**
 * @typedef {Object} SROCoord
 * @property {number} x - X coordinate
 * @property {number} y - Y coordinate
 * @property {number} z - Z coordinate
 * @property {number} region - Region ID
 * @property {number} [posX] - World position X
 * @property {number} [posY] - World position Y
 */

/**
 * @typedef {Object} xSROMapOptions
 * @property {string} [imgHost='assets/img/silkroad/minimap/'] - Base URL for map tiles
 * @property {number} [initialX=114] - Initial X coordinate
 * @property {number} [initialY=47.25] - Initial Y coordinate
 * @property {number} [initialZ=null] - Initial Z coordinate
 * @property {number} [initialRegion=null] - Initial region
 */

/**
 * Creates a new xSROMap instance
 * @param {string} elementId - The DOM element ID to render the map
 * @param {xSROMapOptions} [options={}] - Configuration options
 * @returns {xSROMapInstance}
 */
export function createXSROMap(elementId, options = {}) {
  const imgHost = options.imgHost || 'assets/img/silkroad/minimap/';
  const initialX = options.initialX ?? 114;
  const initialY = options.initialY ?? 47.25;
  const initialZ = options.initialZ ?? null;
  const initialRegion = options.initialRegion ?? null;

  let map;
  let mapLayer;
  let coordGoBack;
  let lastMarkerSelected;

  const mappingLayers = {};
  const mappingMarkers = {
    npc: {},
    tp: {},
    player: {},
    location: {}
  };
  let mappingShapes = {};

  // Coordinate conversion functions
  const CoordMapToSRO = (latlng) => {
    if (mapLayer === mappingLayers['']) {
      return CoordsGameToSRO({
        posX: (latlng.lng - 135) * 192,
        posY: (latlng.lat - 91) * 192
      });
    }
    return {
      x: (latlng.lng * 192 - 128 * 192) * 10,
      y: (latlng.lat * 192 - 127 * 192) * 10,
      z: mapLayer.options.posZ,
      region: mapLayer.options.region
    };
  };

  const CoordSROToMap = (coords) => {
    let lng, lat;
    if (coords.region > 32767) {
      lng = (128 * 192 + coords.x / 10) / 192;
      lat = (127 * 192 + coords.y / 10) / 192;
      return [lat, lng];
    }
    if (coords.posY && coords.posX) {
      lat = coords.posY / 192 + 91;
      lng = coords.posX / 192 + 135;
    } else {
      lng = (coords.region & 0xff) + coords.x / 1920;
      lat = ((coords.region >> 8) & 0xff) + coords.y / 1920 - 1;
    }
    return [lat, lng];
  };

  const CoordsGameToSRO = (gameCoords) => {
    gameCoords.x = Math.round((Math.abs(gameCoords.posX) % 192.0) * 10.0);
    if (gameCoords.posX < 0.0) gameCoords.x = 1920 - gameCoords.x;
    gameCoords.y = Math.round((Math.abs(gameCoords.posY) % 192.0) * 10.0);
    if (gameCoords.posY < 0.0) gameCoords.y = 1920 - gameCoords.y;
    gameCoords.z = 0;

    const xSector = Math.round((gameCoords.posX - gameCoords.x / 10.0) / 192.0 + 135);
    const ySector = Math.round((gameCoords.posY - gameCoords.y / 10.0) / 192.0 + 92);

    gameCoords.region = (ySector << 8) | xSector;
    return gameCoords;
  };

  // Custom TileLayer with inverted Y
  const SRLayer = L.TileLayer.extend({
    getTileUrl: function (tile) {
      tile.y = -tile.y;
      return L.TileLayer.prototype.getTileUrl.call(this, tile);
    }
  });

  const initLayers = () => {
    map = L.map(elementId, {
      crs: L.CRS.Simple,
      minZoom: 0,
      maxZoom: 9,
      zoomControl: false
    });
    new L.Control.Zoom({ position: 'topright' }).addTo(map);

    L.LatLng.prototype.distanceTo = function (currentPostion) {
      const dx = currentPostion.lng - this.lng;
      const dy = currentPostion.lat - this.lat;
      return Math.sqrt(dx * dx + dy * dy);
    };

    const mapSize = 49152;
    map.fitBounds([
      [0, 0],
      [mapSize, mapSize]
    ]);

    mapLayer = new SRLayer(imgHost + '{z}/{x}x{y}.jpg', {
      attribution: '<a href="#">World Map</a>'
    });
    mappingLayers[''] = mapLayer;

    map.addLayer(mapLayer);
    map.setView([91, 135], 8);

    // Dungeon layers
    mappingLayers['32769'] = new SRLayer(imgHost + 'd/{z}/dh_a01_floor01_{x}x{y}.jpg', {
      attribution: '<a href="#">Donwhang Stone Cave [1F]</a>',
      posZ: 0,
      overlap: [
        new SRLayer(imgHost + 'd/{z}/dh_a01_floor02_{x}x{y}.jpg', {
          attribution: '<a href="#">Donwhang Stone Cave [2F]</a>',
          posZ: 115
        }),
        new SRLayer(imgHost + 'd/{z}/dh_a01_floor03_{x}x{y}.jpg', {
          attribution: '<a href="#">Donwhang Stone Cave [3F]</a>',
          posZ: 230
        }),
        new SRLayer(imgHost + 'd/{z}/dh_a01_floor04_{x}x{y}.jpg', {
          attribution: '<a href="#">Donwhang Stone Cave [4F]</a>',
          posZ: 345
        })
      ]
    });

    mappingLayers['32775'] = new SRLayer(imgHost + 'd/{z}/qt_a01_floor01_{x}x{y}.jpg', {
      attribution: '<a href="#">Underground Level 1 of Tomb of Qui-Shin [B1]</a>'
    });
    mappingLayers['32774'] = new SRLayer(imgHost + 'd/{z}/qt_a01_floor02_{x}x{y}.jpg', {
      attribution: '<a href="#">Underground Level 2 of Tomb of Qui-Shin [B2]</a>'
    });
    mappingLayers['32773'] = new SRLayer(imgHost + 'd/{z}/qt_a01_floor03_{x}x{y}.jpg', {
      attribution: '<a href="#">Underground Level 3 of Tomb of Qui-Shin [B3]</a>'
    });
    mappingLayers['32772'] = new SRLayer(imgHost + 'd/{z}/qt_a01_floor04_{x}x{y}.jpg', {
      attribution: '<a href="#">Underground Level 4 of Tomb of Qui-Shin [B4]</a>'
    });
    mappingLayers['32771'] = new SRLayer(imgHost + 'd/{z}/qt_a01_floor05_{x}x{y}.jpg', {
      attribution: '<a href="#">Underground Level 5 of Tomb of Qui-Shin [B5]</a>'
    });
    mappingLayers['32770'] = new SRLayer(imgHost + 'd/{z}/qt_a01_floor06_{x}x{y}.jpg', {
      attribution: '<a href="#">Underground Level 6 of Tomb of Qui-Shin [B6]</a>'
    });

    const jobPath = imgHost + 'd/{z}/rn_sd_egypt1_01_{x}x{y}.jpg';
    mappingLayers['32784'] = new SRLayer(jobPath, {
      attribution: '<a href="#">Temple</a>'
    });
    mappingLayers['32783'] = new SRLayer(imgHost + 'd/{z}/rn_sd_egypt1_02_{x}x{y}.jpg', {
      attribution: '<a href="#">Sanctum of Seth</a>'
    });
    mappingLayers['32782'] = new SRLayer(jobPath, {
      attribution: '<a href="#">Sanctum of Haroeris</a>'
    });
    mappingLayers['32781'] = new SRLayer(jobPath, {
      attribution: '<a href="#">Sanctum of Isis</a>'
    });
    mappingLayers['32780'] = new SRLayer(jobPath, {
      attribution: '<a href="#">Sanctum of Anubis</a>'
    });
    mappingLayers['32779'] = new SRLayer(jobPath, {
      attribution: '<a href="#">Sanctum of Blue Eye</a>'
    });
    mappingLayers['32785'] = new SRLayer(imgHost + 'd/{z}/fort_dungeon01_{x}x{y}.jpg', {
      attribution: '<a href="#">Cave of Meditation [1F]</a>'
    });
    mappingLayers['32786'] = new SRLayer(imgHost + 'd/{z}/flame_dungeon01_{x}x{y}.jpg', {
      attribution: '<a href="#">Flame Mountain</a>'
    });
    mappingLayers['32787'] = new SRLayer(imgHost + 'd/{z}/rn_jupiter_02_{x}x{y}.jpg', {
      attribution: "<a href=\"#\">The Earth's Room</a>"
    });
    mappingLayers['32788'] = new SRLayer(imgHost + 'd/{z}/rn_jupiter_03_{x}x{y}.jpg', {
      attribution: "<a href=\"#\">Yuno's Room</a>"
    });
    mappingLayers['32789'] = new SRLayer(imgHost + 'd/{z}/rn_jupiter_04_{x}x{y}.jpg', {
      attribution: "<a href=\"#\">Jupiter's Room</a>"
    });
    mappingLayers['32790'] = new SRLayer(imgHost + 'd/{z}/rn_jupiter_01_{x}x{y}.jpg', {
      attribution: '<a href="#">Zealots Hideout</a>'
    });
    mappingLayers['32793'] = new SRLayer(imgHost + 'd/{z}/RN_ARABIA_FIELD_02_BOSS_{x}x{y}.jpg', {
      attribution: "<a href=\"#\">Kalia's Hideout</a>"
    });
  };

  const initControls = () => {
    // Check if L.easyButton is available (optional dependency)
    if (typeof L.easyButton === 'function') {
      L.easyButton({
        states: [
          {
            icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 576" style="vertical-align:middle"><path fill="#5b5b5b" d="M444.52 3.52L28.74 195.42c-47.97 22.39-31.98 92.75 19.19 92.75h175.91v175.91c0 51.17 70.36 67.17 92.75 19.19l191.9-415.78c15.99-38.39-25.59-79.97-63.97-63.97z"/></svg>',
            title: 'Go Back',
            onClick: function () {
              setView(coordGoBack);
            }
          }
        ]
      }).addTo(map);
    }
  };

  const initEvents = () => {
    map.on('dblclick', function (e) {
      const coord = CoordMapToSRO(e.latlng);
      let content =
        '[<b> X:' +
        coord.x +
        ' , Y:' +
        coord.y +
        ' , Z:' +
        coord.z +
        ' , Region: ' +
        coord.region +
        (coord.region <= 32767 ? ' (' + (coord.region & 0xff) + ',' + (coord.region >> 8) + ')' : '') +
        ' </b>]';
      if (coord.region <= 32767)
        content = '(<b> PosX:' + coord.posX + ' , PosY:' + coord.posY + ' </b>)<br>' + content;

      const copyLink =
        '<a class="leaflet-popup-copy-button" title="Copy Link" href="#" onClick="if(window.xSROMap)window.xSROMap.LinkToClipboard(' +
        coord.x +
        ',' +
        coord.y +
        ',' +
        coord.z +
        ',' +
        coord.region +
        ')"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 576" style="vertical-align:middle"><path d="M320 448v40c0 13.255-10.745 24-24 24H24c-13.255 0-24-10.745-24-24V120c0-13.255 10.745-24 24-24h72v296c0 30.879 25.121 56 56 56h168zm0-344V0H152c-13.255 0-24 10.745-24 24v368c0 13.255 10.745 24 24 24h272c13.255 0 24-10.745 24-24V128H344c-13.2 0-24-10.8-24-24zm120.971-31.029L375.029 7.029A24 24 0 0 0 358.059 0H352v96h96v-6.059a24 24 0 0 0-7.029-16.97z"/></svg></a>';

      L.popup().setLatLng(e.latlng).setContent(copyLink + content).openOn(map);
    });

    map.on('pm:create', function (e) {
      const shape = e.layer;
      shape.xMap = { layer: mapLayer, type: e.shape, id: shape._leaflet_id };
      if (e.shape === 'Line') shape.xMap.type = 'Polyline';
      addShapeEditListener(shape);
    });

    map.on('pm:remove', function (f) {
      delete mappingShapes[f.layer.xMap.id];
    });
  };

  const setInitialView = (coord) => {
    const GET = (parameter) => {
      const items = location.search.substr(1).split('&');
      for (let i = 0; i < items.length; i++) {
        const tmp = items[i].split('=');
        if (tmp[0] === parameter) return decodeURIComponent(tmp[1]);
      }
      return null;
    };

    const x = parseFloat(GET('x'));
    const y = parseFloat(GET('y'));

    if (!isNaN(x) && !isNaN(y)) {
      const z = parseFloat(GET('z'));
      const r = parseFloat(GET('region'));
      if (!isNaN(z) && !isNaN(r)) setView(fixCoords(x, y, z, r));
      else setView(fixCoords(x, y));
    } else {
      setView(coord);
    }
  };

  const setMapLayer = (tileLayer) => {
    if (tileLayer == null) return;
    if (mapLayer !== tileLayer) {
      map.eachLayer(function (layer) {
        map.removeLayer(layer);
      });

      mapLayer = tileLayer;
      map.addLayer(mapLayer);

      lastMarkerSelected = null;

      for (const type in mappingMarkers) {
        for (const id in mappingMarkers[type]) {
          const marker = mappingMarkers[type][id];
          if (marker.options.xMap.layer === mapLayer) {
            marker.addTo(map);
          }
        }
      }

      for (const id in mappingShapes) {
        const shape = mappingShapes[id];
        if (shape.xMap.layer === mapLayer) {
          shape.addTo(map);
        }
      }
    }
  };

  const getLayer = (coord) => {
    if (coord.region > 32767) {
      let layer = mappingLayers['' + coord.region];
      if (layer) {
        if (layer.options.overlap) {
          const layers = layer.options.overlap;
          for (let i = 0; i < layers.length; i++) {
            if (coord.z < layers[i].options.posZ) break;
            layer = layers[i];
          }
        } else {
          layer.options.posZ = 0;
        }
        layer.options.region = coord.region;
      }
      return layer;
    }
    return mappingLayers[''];
  };

  const setView = (coord) => {
    coordGoBack = coord;
    setMapLayer(getLayer(coord));
    map.panTo(CoordSROToMap(coord), 8);
  };

  const flyView = (coord) => {
    coordGoBack = coord;
    setMapLayer(getLayer(coord));
    map.flyTo(CoordSROToMap(coord), 8, { duration: 2.5 });
  };

  const fixCoords = (x, y, z = null, region = null) => {
    if (region < 0) region += 65536;
    if (region == null) {
      return CoordsGameToSRO({ posX: x, posY: y });
    }
    return { x, y, z, region };
  };

  const toClipboard = (text) => {
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(text);
    } else {
      const e = document.createElement('textarea');
      e.value = text;
      document.body.appendChild(e);
      e.select();
      document.execCommand('copy');
      document.body.removeChild(e);
    }
  };

  const addShapeEditListener = (shape) => {
    mappingShapes[shape.xMap.id] = shape;

    if (shape.xMap.type === 'Marker') {
      shape.on('click', function (e) {
        const coord = CoordMapToSRO(e.latlng);
        let content = '[<b> X:' + coord.x + ' , Y:' + coord.y + ' , Z:' + coord.z + ' , Region: ' + coord.region + ' </b>]';
        if (coord.region <= 32767)
          content = '(<b> PosX:' + Math.round(coord.posX) + ' , PosY:' + Math.round(coord.posY) + ' </b>)<br>' + content;
        content =
          (shape.xMap.desc ? shape.xMap.desc : '<b>&lt; Marker ID:' + shape.xMap.id + ' &gt;</b>') + '<br>' + content;
        L.popup().setLatLng(e.latlng).setContent(content).openOn(map);
      });
    } else if (shape.xMap.type === 'Polygon' || shape.xMap.type === 'Polyline') {
      if (shape.xMap.desc) {
        shape.on('click', function (e) {
          L.popup().setLatLng(e.latlng).setContent(shape.xMap.desc).openOn(map);
        });
      }
    }

    shape.on('pm:edit', function (f) {
      mappingShapes[f.target.xMap.id] = f.target;
    });

    shape.on('pm:vertexremoved', function (f) {
      if (f.target._latlngs.length === 0) delete mappingShapes[f.target.xMap.id];
    });
  };

  // Initialize
  initLayers();
  initControls();
  initEvents();

  if (typeof window !== 'undefined') {
    window.onload = () => setInitialView(fixCoords(initialX, initialY, initialZ, initialRegion));
  } else {
    setInitialView(fixCoords(initialX, initialY, initialZ, initialRegion));
  }

  // Public API
  const instance = {
    /**
     * Get the underlying Leaflet map instance
     * @returns {L.Map}
     */
    getMap() {
      return map;
    },

    /**
     * Set zoom limits
     * @param {number} minZoom
     * @param {number} maxZoom
     */
    SetZoomLimit(minZoom, maxZoom) {
      if (minZoom < 0) minZoom = 0;
      if (maxZoom > 9) maxZoom = 9;
      if (minZoom > maxZoom) {
        const temp = minZoom;
        minZoom = maxZoom;
        maxZoom = temp;
      }
      map.options.minZoom = minZoom;
      map.options.maxZoom = maxZoom;
    },

    /**
     * Set the view instantly
     * @param {number} x
     * @param {number} y
     * @param {number} [z=null]
     * @param {number} [region=null]
     */
    SetView(x, y, z = null, region = null) {
      if (lastMarkerSelected) {
        L.DomUtil.removeClass(lastMarkerSelected._icon, 'leaflet-marker-selected');
        lastMarkerSelected = null;
      }
      setView(fixCoords(x, y, z, region));
    },

    /**
     * Fly to a location with animation
     * @param {number} x
     * @param {number} y
     * @param {number} [z=null]
     * @param {number} [region=null]
     */
    FlyView(x, y, z = null, region = null) {
      if (lastMarkerSelected) {
        L.DomUtil.removeClass(lastMarkerSelected._icon, 'leaflet-marker-selected');
        lastMarkerSelected = null;
      }
      flyView(fixCoords(x, y, z, region));
    },

    /**
     * Add an NPC marker
     * @param {string|number} id
     * @param {string} html
     * @param {number} x
     * @param {number} y
     * @param {number} [z=null]
     * @param {number} [region=null]
     */
    AddNPC(id, html, x, y, z = null, region = null) {
      if (!mappingMarkers.npc[id]) {
        const coord = fixCoords(x, y, z, region);
        const iconNPC = new L.Icon({
          iconUrl: imgHost + 'icon/mm_sign_npc.png',
          iconSize: [6, 6],
          iconAnchor: [3, 3],
          popupAnchor: [0, -3]
        });
        const marker = L.marker(CoordSROToMap(coord), { icon: iconNPC, pmIgnore: true, virtual: true }).bindPopup(html);
        const layer = getLayer(coord);
        if (layer === mapLayer) marker.addTo(map);
        marker.options.xMap = { layer, coordinates: coord };
        mappingMarkers.npc[id] = marker;
      }
    },

    /**
     * Navigate to an NPC
     * @param {string|number} id
     * @returns {boolean}
     */
    GoToNPC(id) {
      const marker = mappingMarkers.npc[id];
      if (marker && marker.options.xMap.layer) {
        setView(marker.options.xMap.coordinates);
        if (lastMarkerSelected) {
          lastMarkerSelected._icon.style.zIndex = lastMarkerSelected._icon._leaflet_pos.y;
          L.DomUtil.removeClass(lastMarkerSelected._icon, 'leaflet-marker-selected');
        }
        lastMarkerSelected = marker;
        marker._icon.style.zIndex = Object.keys(mappingMarkers.npc).length;
        L.DomUtil.addClass(marker._icon, 'leaflet-marker-selected');
        return true;
      }
      return false;
    },

    /**
     * Add a teleport marker
     * @param {string} html
     * @param {number} type - 0: gate, 1: fortress, 2: ress, 3: glory, 4: fortress small, 5: ground, 6: tahomet
     * @param {number} x
     * @param {number} y
     * @param {number} [z=null]
     * @param {number} [region=null]
     */
    AddTeleport(html, type, x, y, z = null, region = null) {
      const coord = fixCoords(x, y, z, region);
      let iconNPC;
      switch (type) {
        case 1:
          iconNPC = new L.Icon({
            iconUrl: imgHost + 'icon/fort_worldmap.png',
            iconSize: [23, 45],
            iconAnchor: [12, 17],
            popupAnchor: [0, -17]
          });
          break;
        case 2:
          iconNPC = new L.Icon({
            iconUrl: imgHost + 'icon/strut_revival_gate.png',
            iconSize: [24, 24],
            iconAnchor: [12, 12],
            popupAnchor: [0, -12]
          });
          break;
        case 3:
          iconNPC = new L.Icon({
            iconUrl: imgHost + 'icon/strut_glory_gate.png',
            iconSize: [24, 24],
            iconAnchor: [12, 12],
            popupAnchor: [0, -12]
          });
          break;
        case 4:
          iconNPC = new L.Icon({
            iconUrl: imgHost + 'icon/fort_small_worldmap.png',
            iconSize: [20, 31],
            iconAnchor: [10, 15],
            popupAnchor: [0, -15]
          });
          break;
        case 5:
          iconNPC = new L.Icon({
            iconUrl: imgHost + 'icon/map_world_icontel.png',
            iconSize: [22, 23],
            iconAnchor: [11, 12],
            popupAnchor: [0, -12]
          });
          break;
        case 6:
          iconNPC = new L.Icon({
            iconUrl: imgHost + 'icon/tahomet_gate.png',
            iconSize: [26, 28],
            iconAnchor: [13, 14],
            popupAnchor: [0, -14]
          });
          break;
        case 0:
        default:
          iconNPC = new L.Icon({
            iconUrl: imgHost + 'icon/xy_gate.png',
            iconSize: [24, 24],
            iconAnchor: [12, 12],
            popupAnchor: [0, -12]
          });
          break;
      }
      const marker = L.marker(CoordSROToMap(coord), { icon: iconNPC, pmIgnore: true, virtual: true }).bindPopup(html);
      const layer = getLayer(coord);
      if (layer === mapLayer) marker.addTo(map);
      marker.options.xMap = { layer, coordinates: coord };
      const tpId = Object.keys(mappingMarkers.tp).length;
      mappingMarkers.tp[tpId] = marker;
    },

    /**
     * Add a player marker
     * @param {string|number} id
     * @param {string} html
     * @param {number} x
     * @param {number} y
     * @param {number} [z=null]
     * @param {number} [region=null]
     */
    AddPlayer(id, html, x, y, z = null, region = null) {
      if (!mappingMarkers.player[id]) {
        const coord = fixCoords(x, y, z, region);
        const iconNPC = new L.Icon({
          iconUrl: imgHost + 'icon/mm_sign_otherplayer.png',
          iconSize: [6, 6],
          iconAnchor: [3, 3],
          popupAnchor: [0, -3]
        });
        const marker = L.marker(CoordSROToMap(coord), { icon: iconNPC, pmIgnore: true, virtual: true }).bindPopup(html);
        const layer = getLayer(coord);
        if (layer === mapLayer) marker.addTo(map);
        marker.options.xMap = { layer, coordinates: coord };
        mappingMarkers.player[id] = marker;
      }
    },

    /**
     * Move a player marker
     * @param {string|number} id
     * @param {number} x
     * @param {number} y
     * @param {number} [z=null]
     * @param {number} [region=null]
     */
    MovePlayer(id, x, y, z = null, region = null) {
      const marker = mappingMarkers.player[id];
      if (marker && marker.options.xMap.layer) {
        marker.options.xMap.coord = fixCoords(x, y, z, region);
        marker.setLatLng(CoordSROToMap(marker.options.xMap.coord));
        const newLayer = getLayer(marker.options.xMap.coord);
        if (marker.options.xMap.layer !== newLayer) {
          if (newLayer === mapLayer) {
            marker.addTo(map);
          } else if (marker.options.xMap.layer === mapLayer) {
            map.eachLayer(function (layer) {
              if (layer === marker) map.removeLayer(layer);
            });
          }
          marker.options.xMap.layer = newLayer;
        }
      }
    },

    /**
     * Navigate to a player
     * @param {string|number} id
     * @returns {boolean}
     */
    GoToPlayer(id) {
      const marker = mappingMarkers.player[id];
      if (marker && marker.options.xMap.layer) {
        setView(marker.options.xMap.coordinates);
        if (lastMarkerSelected) {
          lastMarkerSelected._icon.style.zIndex = lastMarkerSelected._icon._leaflet_pos.y;
          L.DomUtil.removeClass(lastMarkerSelected._icon, 'leaflet-marker-selected');
        }
        lastMarkerSelected = marker;
        marker._icon.style.zIndex = Object.keys(mappingMarkers.player).length;
        L.DomUtil.addClass(marker._icon, 'leaflet-marker-selected');
        return true;
      }
      return false;
    },

    /**
     * Remove a player marker
     * @param {string|number} id
     */
    RemovePlayer(id) {
      const marker = mappingMarkers.player[id];
      if (marker && marker.options.xMap.layer) {
        if (marker.options.xMap.layer === mapLayer) {
          map.eachLayer(function (layer) {
            if (layer === marker) map.removeLayer(layer);
          });
        }
        delete mappingMarkers.player[id];
      }
    },

    /**
     * Add a location marker
     * @param {string|number} id
     * @param {string} html
     * @param {number} x
     * @param {number} y
     * @param {number} [z=null]
     * @param {number} [region=null]
     */
    AddLocation(id, html, x, y, z = null, region = null) {
      if (!mappingMarkers.location[id]) {
        const coord = fixCoords(x, y, z, region);
        const icon = new L.Icon({
          iconUrl: imgHost + 'icon/wmap_sign_location.gif',
          iconSize: [36, 36],
          iconAnchor: [18, 24],
          popupAnchor: [-1, -16]
        });
        let marker = L.marker(CoordSROToMap(coord), { icon, pmIgnore: true, virtual: true });
        if (html !== '') marker = marker.bindPopup(html);
        const layer = getLayer(coord);
        if (layer === mapLayer) marker.addTo(map);
        marker.options.xMap = { layer, coordinates: coord };
        mappingMarkers.location[id] = marker;
      }
    },

    /**
     * Remove a location marker
     * @param {string|number} id
     */
    RemoveLocation(id) {
      const marker = mappingMarkers.location[id];
      if (marker && marker.options.xMap.layer) {
        if (marker.options.xMap.layer === mapLayer) {
          map.eachLayer(function (layer) {
            if (layer === marker) map.removeLayer(layer);
          });
        }
        delete mappingMarkers.location[id];
      }
    },

    /**
     * Copy a link to the clipboard
     * @param {number} x
     * @param {number} y
     * @param {number} [z=null]
     * @param {number} [region=null]
     */
    LinkToClipboard(x, y, z = null, region = null) {
      const coord = fixCoords(x, y, z, region);
      toClipboard(
        window.location.href.split(/\?|#/)[0] +
          '?x=' +
          coord.x +
          '&y=' +
          coord.y +
          '&z=' +
          coord.z +
          '&region=' +
          coord.region
      );
    },

    /**
     * Show the drawing toolbar (requires Leaflet.pm)
     */
    ShowDrawingToolbar(
      position,
      drawMarker,
      drawCircleMarker,
      drawPolyline,
      drawRectangle,
      drawPolygon,
      drawCircle,
      canEdit,
      canDrag,
      canCut,
      canDelete
    ) {
      if (map.pm) {
        map.pm.addControls({
          position,
          drawMarker,
          drawCircleMarker,
          drawPolyline,
          drawRectangle,
          drawPolygon,
          drawCircle,
          editMode: canEdit,
          dragMode: canDrag,
          cutPolygon: canCut,
          removalMode: canDelete
        });
      }
    },

    /**
     * Hide the drawing toolbar
     */
    HideDrawingToolbar() {
      if (map.pm) {
        map.pm.addControls({
          drawMarker: false,
          drawCircleMarker: false,
          drawPolyline: false,
          drawRectangle: false,
          drawPolygon: false,
          drawCircle: false,
          editMode: false,
          dragMode: false,
          cutPolygon: false,
          removalMode: false
        });
      }
    },

    /**
     * Add a drawing shape
     * @param {string} type - 'Marker', 'Polyline', 'Polygon', 'Circle'
     * @param {Array} param1 - Coordinates
     * @param {*} [param2=null] - Description or radius
     */
    AddDrawingShape(type, param1, param2 = null) {
      let shape;
      switch (type) {
        case 'Marker': {
          const coord = fixCoords(param1[0], param1[1], param1[2], param1[3]);
          shape = L.marker(CoordSROToMap(coord), { virtual: true });
          shape.xMap = { layer: getLayer(coord) };
          if (param2 != null) shape.xMap.desc = param2;
          break;
        }
        case 'Polyline':
        case 'Polygon': {
          const latlngs = [];
          for (let i = 0; i < param1.length; i++) {
            latlngs.push(CoordSROToMap(fixCoords(param1[i][0], param1[i][1], param1[i][2], param1[i][3])));
          }
          shape = type === 'Polyline' ? L.polyline(latlngs, { virtual: true }) : L.polygon(latlngs, { virtual: true });
          shape.xMap = { layer: getLayer(fixCoords(param1[0][0], param1[0][1], param1[0][2], param1[0][3])) };
          if (param2 != null) shape.xMap.desc = param2;
          break;
        }
        case 'Circle': {
          const coord = fixCoords(param1[0], param1[1], param1[2], param1[3]);
          shape = L.circle(CoordSROToMap(coord), param2 / 192, { virtual: true });
          shape.xMap = { layer: getLayer(coord) };
          break;
        }
        default:
          return;
      }
      shape.xMap.type = type;
      shape.xMap.id = Date.now();
      mappingShapes[shape.xMap.id] = shape;

      if (shape.xMap.layer === mapLayer) shape.addTo(map);
      addShapeEditListener(shape);
    },

    /**
     * Get all drawing shapes from the current layer
     * @returns {Array}
     */
    GetDrawingShapes() {
      const shapes = [];
      for (const id in mappingShapes) {
        const shape = mappingShapes[id];
        if (shape.xMap.layer === mapLayer) shapes.push(shape);
      }
      return shapes;
    },

    /**
     * Convert Leaflet LatLng to SRO coordinates
     * @param {L.LatLng} latlng
     * @returns {SROCoord}
     */
    ConvertLatLngToCoords(latlng) {
      return CoordMapToSRO(latlng);
    },

    /**
     * Clear all drawing shapes
     */
    ClearDrawingShapes() {
      for (const id in mappingShapes) {
        const shape = mappingShapes[id];
        if (shape.xMap.layer === mapLayer) {
          map.eachLayer(function (layer) {
            if (layer === shape) map.removeLayer(layer);
          });
        }
      }
      mappingShapes = {};
    }
  };

  return instance;
}

// Export coordinate utility functions
export const CoordinateUtils = {
  /**
   * Convert game coordinates to SRO internal coordinates
   * @param {number} posX
   * @param {number} posY
   * @returns {SROCoord}
   */
  gameToSRO(posX, posY) {
    const coords = { posX, posY };
    coords.x = Math.round((Math.abs(posX) % 192.0) * 10.0);
    if (posX < 0.0) coords.x = 1920 - coords.x;
    coords.y = Math.round((Math.abs(posY) % 192.0) * 10.0);
    if (posY < 0.0) coords.y = 1920 - coords.y;
    coords.z = 0;

    const xSector = Math.round((posX - coords.x / 10.0) / 192.0 + 135);
    const ySector = Math.round((posY - coords.y / 10.0) / 192.0 + 92);

    coords.region = (ySector << 8) | xSector;
    return coords;
  },

  /**
   * Convert SRO coordinates to game world coordinates
   * @param {number} x
   * @param {number} y
   * @param {number} region
   * @returns {{posX: number, posY: number}}
   */
  sroToGame(x, y, region) {
    const xSector = region & 0xff;
    const ySector = (region >> 8) & 0xff;
    return {
      posX: (xSector - 135) * 192 + x / 10,
      posY: (ySector - 91) * 192 + y / 10
    };
  }
};

// Default export
export default createXSROMap;
